...
function selectByCondition() {
    ...
    $.ajax({
        url: getContextPath() + "/base/getlowInsuredDate?peopleName="+peopleName+"&page="+page+"&pagesize="+pagesize,
        type: 'POST',
        dataType: 'json',
...
{
    $.ligerDialog.open(
        {   url: getContextPath()+'/base/addlowInsured',
            height: 520,
            width:760,
            title:'添加低保户人员信息',
            buttons: [
                { text: '确定', onclick: function (item, dialog) {
                    ...
                    if (form.valid()) {
                        $.ligerDialog.confirm('是否要添加', function (type) {
                            ...
                            if (type) {
                                $.ajax({
                                    url: getContextPath()+'/base/addlowInsuredData',
                                    type: 'POST',
                                    dataType: 'json',
                                    data: {"emp": emp},
...
function getSelected(rowid)
{
    ...
    $.ligerDialog.open({
        ...
        title: "&nbsp;修改低保户人员信息",
        url: getContextPath()+'/base/updatelowInsured?id='+obj.id,buttons: [
            { text: '确定', onclick: function (item, dialog) {
                var data = {};
                dialog.frame.$("input,select,textarea").each(function () {
                    var name = $(this).attr("name");
                ...
                if (form.valid()) {
                    $.ligerDialog.confirm('是否要修改', function (type) {
                        if (type) {
                            $.ajax({
                                url: getContextPath() + "/base/updatelowInsuredDate",
                                type: 'POST',
                                dataType: 'json',
                                data: {"emp": emp},
                                async: false, //同步处理后面才能处理新添加的series
...
function readSelected(rowid){
    ...
    $.ligerDialog.open({
        ...
        width: 760,
        title: "&nbsp;查看低保户人员信息",
        url:  getContextPath()+'/base/checklowInsured?id='+obj.id,isDrag:true,buttons: [], isResize: true
...
function deleteRow(rowid) {
    ...
    $.ligerDialog.confirm('是否确定删除数据', function (type) {
        if (type){
            $.ajax({
                url: getContextPath() + "/base/deletelowInsured?id="+obj.id,
                type: 'POST',
                dataType: 'json',
                contentType: "application/x-www-form-urlencoded; charset=utf-8",
                success: function (data) {
...
function f_alert() {
    ...
    $.ligerDialog.confirm('是否确定删除数据', function (type) {
        if (type){
            $.ajax({
                url: getContextPath() + "/base/deletelowInsuredAll?box=" + box,
                type: 'POST',
                dataType: 'json',
                contentType: "application/x-www-form-urlencoded; charset=utf-8",
                success: function (data) {
