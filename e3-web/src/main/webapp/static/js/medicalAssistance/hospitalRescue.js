function selectByCondition() {
    ...
    $.ajax({
        url: getContextPath() + "/hospitalRescue/indexDate?h_type="+h_type+"&page="+page+"&pagesize="+pagesize,
        type: 'POST',
        dataType: 'json',
...
/*添加数据、提交*/
function f_open() {
    $.ligerDialog.open({title:'&nbsp;添加住院救助信息',
        url:getContextPath()+'/hospitalRescue/addHospitalRescue', height: 500, width: 740,isDrag:true,
        buttons: [
...
function getSelected(rowid)
{
    ...
    $.ligerDialog.open({
        ...
        title: "&nbsp;修改特定优抚人员信息",
        url: getContextPath()+'/hospitalRescue/updateHospitalRescue?id='+obj.id,buttons: [
            { text: '确定', onclick: function (item, dialog) {
                var data = {};
                ...
                $.ligerDialog.confirm('是否要修改', function (type) {
                    if (type) {
                        $.ajax({
                            url: getContextPath() + "/hospitalRescue/updateHospitalRescueData",
                            type: 'POST',
                            dataType: 'json',
                            data: {"emp": emp},
