function selectByCondition() {
    ...
    $.ajax({
        url: getContextPath() + "/participationRescue/indexDate?peopleName="+peopleName+"&page="+page+"&pagesize="+pagesize,
        type: 'POST',
        dataType: 'json',
...
{
    $.ligerDialog.open(
        {   url: getContextPath()+'/participationRescue/addParticipationRescueData',
            height: 520,
            width:750,
            title:'&nbsp;添加参合保障信息',
            buttons: [
...
function getSelected(rowid)
{
    ...
    $.ligerDialog.open({
        ...
        title: "&nbsp;修改特定优抚人员信息",
        url: getContextPath()+'/participationRescue/updateParticipationRescue?id='+obj.id,buttons: [
            { text: '确定', onclick: function (item, dialog) {
                var data = {};
                dialog.frame.$("input,select,textarea").each(function () {
                    var name = $(this).attr("name");
                ...
                $.ligerDialog.confirm('是否要修改', function (type) {
                    if (type) {
                        $.ajax({
                            url: getContextPath() + "/participationRescue/updateParticipationRescueData",
                            type: 'POST',
                            dataType: 'json',
                            data: {"emp": emp},
                            async: false, //同步处理后面才能处理新添加的series
